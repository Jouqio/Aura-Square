...
const THEME_PRESETS: {
  ...
  {
    value: "celestial-aura",
    labelKey: "theme.celestial_aura",
    fallback: "Celestial Aura",
    bg: "#081028",
    card: "#111827",
    primary: "#38bdf8",
  },
...
export default function SettingsPage(): React.JSX.Element {
  ...
  const { installed, canPromptInstall, isIosManualInstall, promptInstall } =
    useInstallPrompt();
  const [installOutcome, setInstallOutcome] = useState<"dismissed" | null>(
    null,
  );
    ...
    try {
      ...
      setBackupError(
        err instanceof Error && err.message
          ? err.message
          : t("settings.backup_error_read", {
              defaultValue: "Gagal membaca file. Pastikan file tidak rusak.",
            }),
      );
    }
  };
      ...
      >
          ...
          <motion.div variants={rowV}>
            <p className="text-xs font-semibold text-white/30 uppercase tracking-widest px-1 mb-2">
              {t("settings.install_section", {
                defaultValue: "Install Aplikasi",
              })}
            </p>

            {canPromptInstall ? (
              <div
                className="rounded-2xl bg-aura-950/50 border border-aura-800/40 p-4
                flex items-center gap-3"
              >
                <div
                  className="w-10 h-10 rounded-xl bg-aura-800/40 flex items-center
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-white">
                    {t("settings.install_title", {
                      defaultValue: "Pasang Aura Square",
                    })}
                  </p>
                  <p className="text-xs text-white/40 mt-0.5 leading-relaxed">
                    {installOutcome === "dismissed"
                      ? t("settings.install_dismissed", {
                          defaultValue:
                            "Kamu bisa pasang lagi kapan saja dari sini.",
                        })
                      : t("settings.install_desc", {
                          defaultValue:
                            "Main lebih cepat dan mulus — pasang sebagai aplikasi di perangkatmu.",
                        })}
                  </p>
                </div>
                <button
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <p className="text-sm font-bold text-white">
                    {t("settings.install_ios_title", {
                      defaultValue: "Cara pasang di iPhone/iPad",
                    })}
                  </p>
                </div>
                <div className="flex flex-col gap-1 text-xs text-white/50 leading-relaxed">
                  <span>
                    {t("settings.install_ios_step1", {
                      defaultValue: "1. Tap ikon Bagikan di Safari",
                    })}
                  </span>
                  <span>
                    {t("settings.install_ios_step2", {
                      defaultValue: '2. Pilih "Tambah ke Layar Utama"',
                    })}
                  </span>
                  <span>
                    {t("settings.install_ios_step3", {
                      defaultValue: '3. Tap "Tambah" di pojok kanan atas',
                    })}
                  </span>
                </div>
              </div>
        ...
        <motion.div variants={rowV} className="flex flex-col gap-2">
          ...
          <p className="text-xs text-white/25 px-1">
            {t("theme.autosave_note", {
              defaultValue: "Tema tersimpan otomatis di perangkat ini.",
            })}
          </p>
        </motion.div>
        ...
        <Section title={t("nav.language", { defaultValue: "Bahasa" })}>
          <Row
            label={t("settings.language_app", {
              defaultValue: "Bahasa Aplikasi",
            })}
          >
            <Seg
              options={localeOpts}
        ...
        <Section
          title={`${t("settings.sound_effects")} & ${t("settings.vibration", { defaultValue: "Getaran" })}`}
        >
        ...
        <Section
          title={t("settings.accessibility_section", {
            defaultValue: "Aksesibilitas",
          })}
        >
          <Row
            label={t("settings.reduced_motion", {
              defaultValue: "Kurangi Animasi",
            })}
          >
          ...
          <div className="px-4 py-2.5">
            <p className="text-xs text-white/35 leading-relaxed">
              {t("settings.reduced_motion_desc", {
                defaultValue:
                  "Mengurangi partikel, shockwave, dan animasi gerak besar lainnya. Otomatis aktif juga jika sistem perangkatmu sudah mengatur preferensi gerakan minim.",
              })}
            </p>
          </div>
        </Section>
        ...
        <Section
          title={t("settings.profile_section", { defaultValue: "Profil" })}
        >
          <div className="px-4 py-3">
            ...
            >
              {t("settings.edit_name_avatar", {
                defaultValue: "Edit nama & avatar →",
              })}
            </button>
          </div>
        </Section>
          ...
          <div className="px-4 py-3 flex flex-col gap-2">
            <p className="text-xs text-white/40 leading-relaxed">
              {t("settings.data_note", {
                defaultValue:
                  "Semua data disimpan lokal di perangkat ini. Tidak ada server, tidak ada akun.",
              })}
            </p>
          </div>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-white/80">
                {t("settings.backup_export", {
                  defaultValue: "Simpan Cadangan (Backup)",
                })}
              </p>
              <p className="text-xs text-white/35 mt-0.5 leading-relaxed">
                {t("settings.backup_export_desc", {
                  defaultValue:
                    "Unduh semua progress, skor, dan pencapaianmu sebagai file. Simpan file ini di tempat aman.",
                })}
              </p>
            </div>
            <button
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-white/80">
                {t("settings.backup_import", {
                  defaultValue: "Pulihkan dari Cadangan",
                })}
              </p>
              <p className="text-xs text-white/35 mt-0.5 leading-relaxed">
                {t("settings.backup_import_desc", {
                  defaultValue:
                    "Punya file backup? Pulihkan progress dari sana.",
                })}
              </p>
              {backupError && (
          ...
          <div className="px-4 py-3">
            ...
            >
              {t("settings.clear_data", {
                defaultValue: "Hapus semua data & riwayat",
              })}
            </button>
          </div>
        </Section>
        ...
        >
          {t("settings.footer", {
            defaultValue: "Aura Square v3.0 · Offline-First · MIT License",
          })}
        </motion.div>
      </motion.div>
          ...
          >
            <p className="text-sm text-white/80 text-center mb-2">
              {t("settings.clear_data_confirm", {
                defaultValue: "Hapus semua data?",
              })}
            </p>
            <p className="text-xs text-red-400 text-center mb-5">
              {t("settings.clear_data_warning", {
                defaultValue:
                  "Riwayat, statistik, dan pencapaian akan terhapus permanen.",
              })}
            </p>
          ...
          >
            {restoring ? (
              <div className="flex flex-col items-center gap-3 py-4">
                <div
                  className="w-8 h-8 border-2 border-aura-600 border-t-transparent
                  rounded-full animate-spin"
                />
                <p className="text-sm text-white/70 text-center">
                  {t("settings.backup_success", {
                    defaultValue: "Data berhasil dipulihkan! Memuat ulang...",
                  })}
                </p>
              </div>
            ) : (
              <>
                <p className="text-sm text-white/80 text-center mb-1">
                  {t("settings.backup_confirm_title", {
                    defaultValue: "Pulihkan data ini?",
                  })}
                </p>
                <p className="text-xs text-red-400 text-center mb-4">
                  {t("settings.backup_confirm_warning", {
                    defaultValue:
                      "Data saat ini akan DIGANTI dengan isi file backup. Tindakan ini tidak bisa dibatalkan.",
                  })}
                </p>

                {/* Preview */}
                <div
                  className="rounded-2xl bg-surface-300 border border-surface-400
                  p-3 mb-4 flex flex-col gap-1.5 text-xs"
                >
                  {pendingBackup.summary.playerName && (
                    <div className="flex justify-between">
                      <span className="text-white/40">
                        {t("settings.backup_preview_name", {
                          defaultValue: "Nama pemain",
                        })}
                      </span>
                  ...
                  {pendingBackup.summary.bestScore !== null && (
                    <div className="flex justify-between">
                      <span className="text-white/40">
                        {t("settings.backup_preview_best", {
                          defaultValue: "Skor terbaik",
                        })}
                      </span>
                      <span className="text-white font-mono font-bold">
                        {pendingBackup.summary.bestScore.toLocaleString()}
                      </span>
                    </div>
                  )}
                  {pendingBackup.summary.totalGames !== null && (
                    <div className="flex justify-between">
                      <span className="text-white/40">
                        {t("settings.backup_preview_games", {
                          defaultValue: "Total game",
                        })}
                      </span>
                      <span className="text-white font-medium">
                        {pendingBackup.summary.totalGames}
                      </span>
                    </div>
                  )}
                  {pendingBackup.summary.exportedAt && (
                    <div className="flex justify-between">
                      <span className="text-white/40">
                        {t("settings.backup_preview_date", {
                          defaultValue: "Tanggal backup",
                        })}
                      </span>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 rounded-xl bg-aura-600 hover:bg-aura-500
                      text-sm font-bold text-white transition-colors"
                  >
                    {t("settings.backup_restore_cta", {
                      defaultValue: "Ya, Pulihkan",
                    })}
                  </button>
                </div>
              </>
      ...
      {exportToast && (
        ...
        >
          ...
          <span className="text-sm text-white/80">
            {t("settings.backup_exported_toast", {
              defaultValue: "Backup berhasil diunduh!",
            })}
          </span>
        </motion.div>
      )}
